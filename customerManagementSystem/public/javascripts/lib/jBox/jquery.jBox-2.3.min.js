...
(function(a) {
    a.jBox = function(b, c) {
        ...
        if (!f.isObject) {
            ...
            var N = b.toLowerCase();
            if (N.indexOf("id:") == 0) {
            	//文档模式 IE8标准以下 弹出框 功能不正常 提示 设置 STORY #1422 系统固定 ie的浏览模式 yuan.yw
            	if(a.jBox.iedocalert()){
		        	return;
		        }
            	f.type = "ID"; 
            }
            ...
            else if (N.indexOf("html:") == 0) f.type = "HTML"; else {
                b = "html:" + b;
                f.type = "HTML";
            }
       ...
       // j.push('<td class="jbox-border" style="margin:0px;padding:0px;border:none;"></td>');
        ...
        j.push('<a class="jbox-close" title="' + a.jBox.languageDefaults.close + '" onmouseover="$(this).addClass(\'jbox-close-hover\');" onmouseout="$(this).removeClass(\'jbox-close-hover\');" style="position:absolute; display:block;text-align: center;line-height: 26px;  cursor:pointer; top:' + (0 ) + "px; right:" + (0 ) + "px; width:25px; height:28px;" + (c.showClose ? "" : "display:none;") + '">X</a>');
        if (g) {
            j.push('<div class="jbox-title-panel" style="height:28px;">');
            j.push('<div class="jbox-title' + (c.showIcon == true ? " jbox-title-icon" : c.showIcon == false ? "" : " " + c.showIcon) + '" style=" width:' + i + "; line-height:" + (a.browser.msie ? 27 : 28) + "px; padding-left:" + (c.showIcon ? 18 : 5) + 'px;overflow:hidden;text-overflow:ellipsis;word-break:break-all;">' + (c.title == "" ? "&nbsp;" : c.title) + "</div>");
            j.push("</div>");
        }
       ...
       /* if (c.border > 0) {//wen 2016-02-18
        ...
        a.each(b, function(N, O) {
            ...
            r.push('<div class="jbox-button-panel" style="height:25px;padding:5px 0 5px 0;text-align: center;' + (P ? "" : "display:none;") + '">');
            if (!c.isTip) {
                r.push('<span class="jbox-bottom-text" style="float:left;display:block;line-height:25px;"></span>');
            }
            a.each(O.buttons, function(T, U) {
                r.push('<button class="jbox-button" value="' + U + '" style="' + u + 'margin-left:5px;margin-right:5px;">' + T + "</button>");
              //  r.push('<input type="button" class="jbox-button" value="' + T+ '" style="' + u + '"/>' );
            });
            r.push("</div></div>");
        });
        ...
        var B = function() {
            if (c.isMessager) {
                ...
                o.css({
                    position: "absolute",
                    top: q.offset().top + (c.isTip ? l.scrollTop() : 0)+100,
    ...
    //文档模式 IE8标准以下 提示 设置 STORY #1422 系统固定 ie的浏览模式 yuan.yw
    a.jBox.iedocalert = function() {
    	var msie = /msie/.test(navigator.userAgent.toLowerCase());
        if(msie  && document.documentMode&& document.documentMode<8){//文档模式 限制IE8及以上标准
        	
			var panel = "";
			$("#jbox_ie-alert").remove();
			var obj = $("<div id='jbox_ie-alert'></div>");
			var overlay = $("<div id='jbox_ie-alert-overlay'></div>");
			var iepanel = $("<div id='jbox_ie-alert-panel'>"+ panel +"</div>");
			var docHeight = $(document).height();
			overlay.css("height", docHeight + "px");
			panel = "<p style='font-size:16px'> 由于您现在使用的是非IE标准模式，会导致部分功能无法正常使用，请您按F12，打开开发者工具，然后文档模式选择 当前的IE标准模式.</p>" +
					"<br><div id='jbox_ie-alert-imgdiv'></div>";
			iepanel = $("<div id='jbox_ie-alert-panel'>"+ panel +"</div>");
			$("body").append(obj);
			obj.prepend(iepanel);
			obj.prepend(overlay);
			obj.show();
    ...
    a.jBox.prompt = function(b, c, d, e) {
        ...
        var g = e.icon == "none" ? "height:auto;" : "min-height:30px;padding-top:10px;;" + (a.browser.msie && parseInt(a.browser.version) < 7 ? "height:auto !important;height:100%;_height:30px;" : "height:auto;");
        var h = [];
        h.push("html:");
