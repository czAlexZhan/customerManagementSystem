...
My97DP.prototype = {
    ...
    initQS: function() {
        ...
        if (E > $) E = $; else if (_ == "m" || _ == "s") C = [ -60, -30, 0, 30, 60, -15, 15, -45, 45 ]; else for (H = 0; H < $ + 9; H++) C[H] = $dt[_] - 2 + H;
        for (H = G = 0; H < E; H++) {
...
function getWeek($, B) {
    ...
    if ($dp.weekMethod == "ISO8601") {
        ...
        return Math.round((A.valueOf() - _.valueOf()) / (7 * 864e5)) + 1;
    } else {
        _.setDate(1);
...
function c(_, A) {
    var $ = A;
    if (_ == "M") $ = makeInRange(A, 1, 12); else if (_ == "H") $ = makeInRange(A, 0, 23); else if ("ms".indexOf(_) >= 0) $ = makeInRange(A, 0, 59);
    if ($sdt[_] != A && !callFunc(_ + "changing")) {
        var C = $c.checkRange();
        if (C == 0) sv(_, $); else if (C < 0) B($c.minDate); else if (C > 0) B($c.maxDate);
        $d.okI.disabled = !$c.checkValid($sdt);
...
function _tab(M) {
    ...
    if ($dp.enableKeyboard && isShow) {
        ...
        if (Q >= 37 && Q <= 40) {
            ...
            if ($c.currFocus.type == "button") $c.currFocus.click(); else if ($dp.cal.oldValue == $dp.el[$dp.elProp]) $c.pickDate(); else $c.close();
            $c.currFocus = $dp.el;
        }
